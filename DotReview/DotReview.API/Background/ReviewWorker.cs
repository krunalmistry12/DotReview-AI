...
public class ReviewWorker : BackgroundService
{
        ...
        CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Review comment posted to PR #{PullRequestNumber}.",
                    job.PullRequestNumber);
            }
        ...
        CodeReviewResponse review)
    {
        ...
        comment.AppendLine("## 🤖 DotReview AI Code Review");
        comment.AppendLine();
            ...
            review.Issues.Count == 0)
        {
            ...
            comment.AppendLine();
            comment.AppendLine(
                "The reviewed code looks good.");

            return comment.ToString();
        }

        comment.AppendLine(
            $"### Issues found: **{review.Issues.Count}**");
        ...
        foreach (var issue in review.Issues)
        {
            ...
            comment.AppendLine(
                $"#### {GetSeverityIcon(severity)} {severity} — {category}");
        ...
        comment.AppendLine(
            "_Review generated by DotReview AI._");

        return comment.ToString();
    }

    private static string GetSeverityIcon(
        string severity)
    {
